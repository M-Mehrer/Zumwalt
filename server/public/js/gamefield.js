class Gamefield{
    ...
    setUpShipsRandomly(){ // eslint-disable-line no-unused-vars 
            ...
            for(let currentShipProperty = 0; currentShipProperty < shipProperties.length; currentShipProperty++){
                for(let shipNumber = 0; shipNumber < ships.getShip(currentShipProperty).amount; shipNumber++){	
                    ...
                    while(tmpFreeFields.length > 0 && actualShipNotSetInGameField){
                        let position = this.getRowAndColFromString(tmpFreeFields[getRandomInt(0, tmpFreeFields.length - 1)]); // eslint-disable-line no-undef
                        let row = position[0];
                        let col = position[1];
    ...
    */
    getRowAndColFromString(string){ // eslint-disable-line no-unused-vars 
